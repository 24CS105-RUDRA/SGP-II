...
export default function AdminGalleryPage() {
  ...
  useEffect(() => {
    ...
    setUser(userData)
    console.log('[v0] Admin user loaded:', userData.username)
    fetchEvents()
    setLoading(false)
  }, [router])

  const fetchEvents = async () => {
    console.log('[v0] Admin fetching all gallery events')
    const result = await getAllGalleryEvents()
    console.log('[v0] Admin fetchEvents result - success:', result.success, 'count:', result.data?.length || 0)
    if (result.success && result.data) {
      console.log('[v0] Setting events:', result.data.length)
